...
export default {
  ...
  methods: {
    ...
    async showEditUserDialog(data) {
      ...
      this.file = await this.getFileById(this.editUserForm.id)
      console.log(this.file)
      if (this.file !== null) {
        this.$set(this.editUserForm, 'imgUrl', this.file.name)
        this.editUserForm.fileId = this.file.id
      }
    },
          ...
          if (data.success) {
            ...
            if (this.file === null) {
              await this.$http.post('/list/insert', {
                ...
                filename: this.editUserForm.imgUrl,
                createTime: Date.now(),
                updateTime: Date.now()
              })
            }
